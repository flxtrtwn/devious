...
def docker_compose_build(docker_compose_file: PurePath) -> list[str]:
    return [
        ...
        "compose",
        "-f",
        docker_compose_file.as_posix(),
        "build",
    ]


def docker_run(
    bind_ports: dict[int, int], name: str, detached: bool = True
) -> list[str]:
    cmd = [
        "docker",
        "run",
...
def docker_compose_up(
    docker_compose_yaml: PurePath, detached: bool = True
) -> list[str]:
    cmd = [
        "docker",
        "compose",
...
def docker_compose_exec(
    docker_compose_yaml: PurePath, commands: list[str]
) -> list[str]:
    return [
        "docker",
        "compose",
